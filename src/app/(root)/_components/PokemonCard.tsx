...
function PokemonCard({ pokemon }: PokemonCardProps) {
    ...
    const handleMouseMove = (
        ...
        e.currentTarget.style.backgroundPosition = `${x / 5 + y / 5}%`;
    ...
    return (
        <Link
            ...
            className="relative flex justify-center items-center transition-all duration-100"
        >
            <div
                onMouseMove={handleMouseMove}
                onMouseLeave={handleMouseLeave}
                className={`z-10 absolute w-[244px] h-96 filter-brightness-opacity transition-all duration-100 ${styles.bg_custom_gradient} ${styles.filter_brightness_opacity} ${styles.mix_blend_color_dodge} ${styles.bg_size} ${styles.bg_position}`}
            />
            <li
                ref={liRef}
                className="h-96 w-80 flex flex-col items-center justify-center gap-4 rounded-lg bg-gray-200 shadow-md duration-400 ease-in-out transition transform hover:shadow-lg"
            >
